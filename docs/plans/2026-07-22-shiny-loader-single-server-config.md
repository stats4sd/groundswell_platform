...
**Status:** In Progress (substantially complete) — Tasks 1–4 implemented, reviewed, and committed (see [change log](../change-logs/2026-07-22-shiny-loader-single-server-config.md)); the package branch is merged to `main` and released as v1.2.1, with the app's gitlink bumped in `e5d2fa5`. Task 5's docker harness is built and committed (`6e6e6a2`: `packages/docker-compose.yml`, `Dockerfile`, `entrypoint.sh`) and has demonstrably been run (Linux renv libraries written into the bind-mounted app folders), and a real `SHINY_AUTH_KEY` is set in `.env` matching both R apps. Remaining: record the outcomes of Task 5 Steps 2–4 (monitor e2e, analysis e2e, and the 422 foreign-callback failure mode) — no verification result is documented, and `packages/groundswell_analysis/.env` currently has a trailing `LIVE TEMP` block overriding `URL` back to the staging shiny server, so the local analysis path needs re-pointing before retesting — then write the Task 5 change log entry and flip this line to Completed.
