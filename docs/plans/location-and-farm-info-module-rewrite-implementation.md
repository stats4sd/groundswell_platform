...
## Context notes for whoever resumes this (not required by any task, but worth knowing)

These didn't change any task's code, but came up during design/planning and are easy to lose otherwise.

**1. `has_updated_locations` is currently never set anywhere in the codebase.** Confirmed by grepping the whole app: only the migration (`2025_06_19_085844_add_has_updated_locations_to_teams_table.php`) and `Team::localiseXlsforms()`/`deployDraftForms()` itself reference this column — nothing flips it to `true`. This predates this rewrite (it's true of the legacy `LocationSectionBuilder` too) but means that **after this plan ships, the new builders still won't run automatically in production** — `localiseXlsforms()` is only reachable today via `InitialPilot::mount()` (`app/Filament/App/Pages/PlaceAdaptations/InitialPilot.php:63`), which only fires when that specific page is visited. Wiring something to set `has_updated_locations = true` (e.g. from `LocationLevel`/`Location`/`FarmEntity` save hooks) is a real gap, but a separate piece of work from this plan — flagging it so it isn't mistaken for "done" once these 4 tasks ship.

**2. `Xlsform::syncWithTemplate()` only runs for forms where `has_latest_template` is false**, and that column defaults to `true` at the DB level (`xlsforms` migration). This matters for the admin-panel task that's a prerequisite for any of this to take effect for real: once an admin adds the `Locations`/`Farm Info` `XlsformModule` rows (with `can_be_replaced = true`) to a template, **existing `Xlsform`s already built from that template won't pick up the new modules** until either `has_latest_template` is flipped back to `false` on them (triggering a re-sync on next save) or `syncWithTemplate()` is called on them directly. Worth a one-off note/reminder for whoever does that admin-panel setup — not something this plan's tasks need to handle, since Task 1-4 only add the *capability*, not a backfill.

**3. Local dev DB currently has no `Locations`/`Farm Info` modules on any template, and no `Global location` version at all** — querying `XlsformModule`/`XlsformTemplate` locally showed exactly 2 templates (`Farm Registration Test 1`, `Indicators Test 1`) and 15 modules total, none named `Locations`/`Farm Info`/`Global location`. `Indicators Test 1`'s existing modules, in `default_order`: `metadata`(1), `introduction`(2), `demographics`(3), `practices`(4), `land use`(5), `crops`(6), `crop`(7), `crop_intensification`(8), `livestock`(9), `livestock_intensification`(10), `financial_situation`(11), `debts and aid`(12), `off farm incomes`(13), `metadata_close`(14). `Farm Registration Test 1` only has `metadata`(1). This means: **the Pest tests in this plan are fully self-contained (they build their own template/module fixtures) and will pass without any manual setup**, but end-to-end manual verification (actually deploying a form and seeing the new sections) requires first doing the out-of-scope admin-panel step yourself — e.g. adding a `Locations` module to both test templates at `default_order` 2 (after `metadata`), and a `Farm Info` module to `Indicators Test 1` only, both with `can_be_replaced = true`.

**4. `jr:choice-name()` argument semantics were verified against authoritative sources, not assumed** — worth keeping the citations if this ever needs re-checking: [ODK Form Operators and Functions](https://docs.getodk.org/form-operators-functions/) and the [ODK forum thread on jr:choice-name](https://forum.getodk.org/t/use-of-jr-choice-name-in-xlsform/2697) both confirm the signature is `jr:choice-name(value, '${question_name}')` — second argument is a quoted question reference, not a bare list-name string. This is why the design doc's `loc{n}_type`/`location_level` indirection from the original example CSV was dropped rather than transcribed as-is.
