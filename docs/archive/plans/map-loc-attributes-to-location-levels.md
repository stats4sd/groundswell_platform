...
## Reverted: back to a fully generic `buildLocationAttributes()` (2026-07-16)

Per Dan: teams are not fixed to a "Cluster"/"Group" two-level hierarchy - each team configures its own arbitrary-depth chain of `LocationLevel`s (e.g. District/Sub-district/Village for another team), so a lookup table hardcoded to one team's group names, and a `loc{n}_type` placeholder string, can't generalize. `GROUP_CLUSTER_LOOKUP`/`resolveGroupClusterIds()` (2026-07-15, above) are removed entirely.

- **`buildLocationAttributes()` revised again**: `loc{n}` is now `(string) $current->code` (the matched `Location`'s own `code` column) rather than a hardcoded/looked-up ID or the `"1"` placeholder - flagged in code with a `TODO` since it's undecided whether `loc{n}` should carry `locations.code` or `locations.id` going forward. `loc{n}_type` is now the matched `LocationLevel`'s own `name` (e.g. "Cluster", "District"), not the fixed placeholder string `"Loc{n} name"`.
- **Known consequence, accepted (confirmed with Dan before implementing)**: this team's deployed Farm Registration XLSForm's Enketo cascading selects were relying on `GROUP_CLUSTER_LOOKUP`'s specific internal cluster/group choice-list IDs for `loc1`/`loc2` on app-created farms. Those IDs are not derivable from this app's own `Location`/`LocationLevel` records, so app-created farms' `loc1`/`loc2` will no longer match the deployed choice list's numeric IDs. Reconciling the ODK form/choice list itself (e.g. keying it off `Location.code` instead) is a separate, later piece of work, out of scope here.
- **`resolveLocationFromAttributes()` (read side) is unaffected** - it only ever matched on `loc{n}_name`, never the bare `loc{n}` value, so this change is write-side only.
- **Tests**: the 3 hardcoded-lookup-specific cases in `tests/Feature/Services/OdkFarmEntityServiceLocationTest.php` were removed; the arbitrary-depth chain-walk test now asserts `loc{n}` against each Location's `code` and `loc{n}_type` against the real `LocationLevel` name.
