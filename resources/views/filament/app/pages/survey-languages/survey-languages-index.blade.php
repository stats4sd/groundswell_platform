...
<x-instructions-sidebar>
        ...
        <x-slot:instructions>
            ...
            <div class="mx-12 mb-4">
                <p class="mb-2">
                  {{ t("The first step to set up the survey is to select the country and languages your team will conduct the survey in. Bear in mind that if you change these details later, you may need to review some of the other steps.") }}
                </p>
                ...
                <h5>{{ t("Survey translations") }}</h5>
                <p class="mb-2">
                    {{ t("When you have selected the languages for your survey, you should review the available translations.") }}
                </p>
                <p class="mb-2">
                    {{ t("HOLPA is available in multiple languages. On the translations page, for each of the languages you selected previously, you will need to select the translation to use. Click \"Select translation\" to show the available translations for a language.") }}
                </p>
                <p class="mb-2">
                    {{ t("If there is no existing translation, or the available translations are not suitable for your survey location, you should add a new translation. You may wish to duplicate an existing translation to use as a starting point.") }}
                </p>
                <p class="mb-2">
                    {{ t("To add a new translation, click on \"add new\" and give it a clear name. The translation will appear in the table. You can then use the \"view/edit\" menu to download the translation template. Add the translated text in the indicated column in the xls files, then reupload them using the same menu. Once both forms have translated versions uploaded, the translation will be available for use. When editing or adding a translation, make sure you download the file for the language you wish to edit, and make the changes in that file. If the column headers do not match the original file or if the column for the translated text is left blank, you will see an error message when you upload the files.") }}
                </p>
                <p class="mb-2">
                    {{ t("If you click on view/edit translation, this will allow you to download the xls files with the translated text.") }}
                    {{ t("You can edit translations that your team has added. To do this, download the existing translation files and follow the same process as when adding a translation: make changes in the column for the destination language and upload the edited file through the same menu. Note that the translation is to reflect the text <span class=\"font-semibold\">as written in the HOLPA survey</span>; this is not the appropriate place to customise the content of the survey. Translations added by other teams or from the original ODK forms cannot be directly edited, but you can duplicate the translation to make a new version with your changes.") }}
                </p>
                <p class="mb-2">
                        {{ t("The HOLPA platform will make user-uploaded translations available to all other teams.") }}
                    </p>
                    <p class="instructions_note">
                    <span class="font-semibold"> {{ t("NOTE:") }} </span> {{ t("The platform assumes that you are working within one country. If you are conducting HOLPA across multiple countries, you will need to create separate teams within this platform for each country. Please contact the HOLPA support team if you require assistance with this.") }}
                    </p>
                    <h5>{{ t("Mark this section as complete when:") }}</h5>

                    <p class="mb-2">
                        {{ t("You have selected the country, languages and added a global HOLPA survey translation for each language.") }}
                    </p>
