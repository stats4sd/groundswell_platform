...
<x-instructions-sidebar>
        ...
        <x-slot:instructions>
            ...
            <div class="mx-12 mb-4">
                ...
                <p class="mb-2">
                    {{ t("There are some questions in the survey where the appropriate answer options will be different depending on the location context - for example, questions that ask about crops that are grown on a farm") }}
                    <i>{{ t("should not") }}</i> {{ t("include lots of options for plants that do not grow in the location being surveyed, and") }}
                    <i>{{ t("should") }}</i> {{ t("include the most commonly grown crops in that area. Questions should also reflect the units of measurement that are used in the location.") }}
                </p>
                <p class="mb-2">
                    {{ t("The \"Contextualise choice lists\" has several choice lists to be checked and customised. You can select from the lists on the left hand side, then review the existing options. Options can be removed from the context, so they will not be included in this questionnaire, and you have the option to add new options by clicking the \"add new\" button. For each choice list entry, you will need to add a name and label, then click \"create\", or \"create and add another\" to save the entry.") }}
                </p>

                <p class="mb-2">
                    {{ t("Each list page includes the option to view the questions that will use these answer options. Check these to ensure you provide suitable options.") }}
                </p>
