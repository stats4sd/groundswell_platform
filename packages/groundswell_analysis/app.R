...
ui <- dashboardPage(
  ...
  dashboardHeader(title = "Groundswell Survey Monitoring App", titleWidth = 450, disable = TRUE),
    ...
    tabItems(
        ...
        box(
          ...
          fluidRow(
            conditionalPanel(
              ...
              column(
                width = 12,
                conditionalPanel("output.data_in",
            ...
            conditionalPanel("output.data_in", box(
              fluidRow(
                ...
                selectInput(
                  "monitor_table",
                  "Table:",
                  choices = c("Farmer Completion" =
                                "match", "Form Completion" = "completion"),
                  selected = "match",
                  width = "50%"
                )
              ), width = 12
                         ...
                         box(column(
                           ...
                           column(
                             conditionalPanel(
                               ...
                               fluidRow(
                                 downloadButton("dlx", "Download Form Data as XLSX (including repeats)")
                               )
      ...
      tabItem(
        ...
        conditionalPanel("output.data_in", 
                         box(
                           ...
                           column(
                             width = 12,
                             fluidRow(downloadButton("dl_processed","Download Selected Processed Data as CSV")),
                             fluidRow(downloadButton("dlx_processed","Download All Processed Data as XLSX"))
                           ), width = 6),
                         #,
      ...
      tabItem(tabName = "Enumerator", 
              conditionalPanel(
                ...
                box(column(width = 12,
                           fluidRow(dataTableOutput("enum_summary"))),
                    width = 12)
                
              )),
      #Monitoring results at overall level
      tabItem(tabName = "Monitor", conditionalPanel(
        "output.data_in",
        
        box(
          column(fluidRow(plotlyOutput("Dates")), width =
                   12), width = 12),
        
        box(column(fluidRow(
          selectInput("geo_var", "Select Co-ordinate Variable", choices = c("None" = 0)),
          conditionalPanel("input.geo_var!='0'", leafletOutput("MonitorMap"))
        ),width = 12),
        width = 12),
        box(
          column(
            width = 12,
            fluidRow(
              selectInput("inspect", "Select Farmer to Inspect", choices = c("None" = 0),
                          width = "50%"),
              conditionalPanel(
                "output.data_in",
                ...
                selectInput(
                  "form_i",
                  label = "Select form",
                  choices = c("Global Indicators" = "global", "Women's Form" = "women"),
                  width = "50%")
              ),
        ...
        conditionalPanel("output.data_in", 
                         ...
                         box(
                           ...
                           column(
                             width = 12,
                             fluidRow(
                               selectInput("question", "Select Question", choices = "Please Load Data",
                                           width = "50%")
                             ),
                             fluidRow(
                               ...
                               conditionalPanel(
                                 ...
                                 selectInput(
                                   ...
                                   choices = c("Responses", "Percent of Non-Missing", "Percent of All"),
                                   width = "50%"
                                 )
                               )))),
                         box(
          ...
          box(
            column(width = 12,
                   fluidRow(
                     ...
                     #  ))
                     #  ,
                     selectInput("indicator", "Select Indicator", choices = available_indicators),
                     selectInput(
                       "groupvar",
                     ...
                     downloadButton("dl_indicators","Download Indicator Data as CSV"),
                   )), width = 6),
          box(
            fluidRow(column(htmlOutput("indicator_text"),
                            width = 12)),
            fluidRow(column(withSpinner(
      ...
      tabItem(tabName = "Report", fluidRow(
        conditionalPanel("output.data_in", 
                         box(
                           #selectInput(
                           #  "report_data",
                           ...
                           selectInput("regen",label="", choices = c( "Generate new report"=0)),
                           conditionalPanel(
                             "input.regen==0",
                             actionButton("genBaiscReport", "Generate Project Summary Report")
                           )
...
server <- function(input, output,session) {
  ...
  messages<-reactiveValues(data1=NULL,data2=NULL,data3=NULL,data_message="",progress=NULL,loading=0,
                           project_code=project_code)
  data1<-reactiveValues(flag=FALSE,data=0,forms=NULL,progress=0,
                        selected_forms=NULL,selected_language="default",data0=NULL)
  ...
  outpath$outpath <- paste0("output/reports")
    ...
    output$DataMenu <- renderMenu({
      sidebarMenu(
        menuItem(tabName = "Data", text =  language_switch("tabName_Data",input$language))
      )
    })
   
    output$SummariseMenu <- renderMenu({
      sidebarMenu(
        menuItem(tabName = "Summarise", text =  language_switch("tabName_Summarise",input$language))
      )
    })
    output$View2Menu <- renderMenu({
      sidebarMenu(
        menuItem(tabName = "View2", text =  language_switch("tabName_View2",input$language))
      )
    })    
    output$IndicatorsMenu <- renderMenu({
      sidebarMenu(
        menuItem(tabName = "Indicators", text =  language_switch("tabName_Indicators",input$language))
      )
    })
      ...
      if(proj_info$pid==0){
        output$data_message1<-renderText("No currently active ODK Central data collection activities for this project.
        ...
        languages<-c("english_(en)","español_(es)","nepali_(ne)","français_(fr)")
        names(languages)<-c("English","Español","Nepali","Français")
        
        language_proj <- data_directory%>%
          filter(project==project_code) %>%
          select(Language) %>%
          as.character() %>% str_split("; ") %>% unlist()%>% trimws() 
        updateSelectInput(session=session,"language",choices=languages[names(languages)%in%language_proj])
    ...
    observeEvent(input$language,{
      ...
      output$lang_message<-renderText(language_switch("updateLanguage",input$language))  
      
      
      #update interface after changing languages      
      
      updateSelectInput(session = session,
                        inputId = "country",label=language_switch("select_Country",input$language))     
      
      updateSelectInput(session = session,"language",label=language_switch("select_Language",input$language))
      
      updateActionButton(session = session,"confirm",label = language_switch("action_confirm",input$language))
      
      #creating a named vector for the selection list to update language
      ch1<-c("match",
             "completion")
      names(ch1)<-c(language_switch("select_monitor_table_match",input$language),
                    language_switch("select_monitor_table_completion",input$language))
      
      updateSelectInput(session = session,"monitor_table",language_switch("select_monitor_table",input$language),
                        choices=ch1)
      ...
      ch2<-c("global",
             "women",
             "reg")
      names(ch2)<-c(language_switch("select_form1_global",input$language),
                    language_switch("select_form1_women",input$language),
                    language_switch("select_form1_reg",input$language))
      
      
      updateSelectInput(session = session,"form1",label = language_switch("select_form1",input$language),
                        choices=ch2)
      ...
      updateSelectInput(session = session,"enums",language_switch("select_enums",input$language),
                        choices=language_switch("select_enums_All",input$language))
      updateDateRangeInput(session = session,"dates",language_switch("dateRange_dates",input$language))
      
      ch3<-0
      names(ch3)<-language_switch("select_geo_var_None",input$language)
      
      updateSelectInput(session = session,"geo_var",language_switch("select_geo_var",input$language),
                        choices=ch3)
      
      updateSelectInput(session = session,"inspect",language_switch("select_inspect",input$language),
                        choices=ch3)
      
      updateSelectInput(session = session,"form_i",label = language_switch("select_form1",input$language),
                        choices=ch2[-3])
      updateSelectInput(session = session,"form2",label = language_switch("select_form1",input$language),
                        choices=ch2)
      
      updateSelectInput(session = session,"question",language_switch("select_question",input$language),choices = "Error - Please Load Data")
      ...
      updateSelectInput(session = session, "indicator", label = language_switch("select_indicator",
                                                                                input$language))
      #  ch4 <- c(
      #    "raw",
      #    "processed"
      ...
      print(project_code)
      if(!is.na(data_directory$livestock[data_directory$project==project_code])){
        ch5 <- c(ch5, "livestock")
        
        names(ch5) <- c(language_switch("select_cleandat_global",input$language),
      ...
      updateSelectInput(session = session, "cleandat", label = language_switch("select_cleandat",
                                                                               input$language),
                        choices = ch5)
      ...
      updateSelectInput(session = session, "genBaiscReport", label = language_switch("action_genBasicReport",
                                                                                     input$language))
      updateSelectInput(session = session, "printBasicReport", label = language_switch("action_printBasicReport",
                                                                                       input$language))
      updateSelectInput(session = session, "printBasicReport2", label = language_switch("action_printBasicReport",
                                                                                        input$language))
      ...
      updateSelectInput(session = session, "groupvar", label = language_switch("select_groupvar",input$language),
                        choices = ch_ig)
    ...
    observeEvent(input$language,{
      ...
      messages$progress<-data_directory$Status[data_directory$project==project_code]
    ...
    observeEvent(input$confirm,{
      ...
      withProgress(message = language_switch("output_data_message_loading",input$language), value = 0, {
        ...
        if( messages$progress==1){
          ...
          data1$data0$full_form <- try(form_schema_ext(fid = Sys.getenv("form_id")) )
          incProgress(0.1)
          data1$data0$full_form_women <- try(form_schema_ext(fid = Sys.getenv("women_form_id")) )
          incProgress(0.1)
          data1$data0$full_form_registration <- try(form_schema_ext(fid = Sys.getenv("reg_form_id")) )
          incProgress(0.1)
          data1$data0$enum_form <- try(form_schema_ext(fid = Sys.getenv("rnum_form_id")) )
          ...
          if(class(data1$data0$full_form)[1]!="try-error"){
            ...
            if(class(data1$data0$enum_form)[1]!="try-error"  & "Enumerator_List"%in%ruODK::entitylist_list()){
              data1$data0$enum_flag<-TRUE
              
            }
            
            #get language names
            data1$data0$languages<-str_remove_all(str_subset(colnames( data1$data0$full_form),"label_"),"label_")
          ...
          data1$data0$global<-read_clean_data(fid=Sys.getenv("form_id"),language=input$language,
                                              full_form=form_schema_ext(fid=Sys.getenv("form_id")))
          
          
          #create message saying either fail or how many records in data
          messages$data1<-ifelse(class(data1$data0$global$full_data)[1]!="try-error",
                                 paste(language_switch("message1_data_success",input$language),
                                       nrow(data1$data0$global$full_data), language_switch("message1_data_records",input$language)),
                                 language_switch("message1_data_failure",input$language))
          
          #repeat for womens data
          
          data1$data0$women<- read_clean_data(fid=Sys.getenv("women_form_id"),language=input$language,
                                              full_form=form_schema_ext(fid=Sys.getenv("women_form_id")))
          
          messages$data2<-ifelse(class(data1$data0$women$full_data)[1]!="try-error",
                                 paste(language_switch("message2_data_success",input$language),
                                       nrow(data1$data0$women$full_data), language_switch("message1_data_records",input$language)),
          ...
          data1$data0$global$full_data$partner<-names(proj_list)[proj_list==project_code]
          
          if("livestock_owners" %in% data1$data0$global$full_form$name){
            data1$data0$indicators <- derive_indicators(global = data1$data0$global$full_data,
          ...
          #repeat for reg data
          data1$data0$reg<- read_clean_data(fid=Sys.getenv("reg_form_id"),language=input$language,
                                            full_form=form_schema_ext(fid=Sys.getenv("reg_form_id")))
          
          messages$data3<-ifelse(class(data1$data0$reg$full_data)[1]!="try-error",
                                 paste(language_switch("message3_data_success",input$language),
                                       nrow(data1$data0$reg$full_data), language_switch("message1_data_records",input$language)),
          ...
          if(    data1$data0$enum_flag==TRUE){
            ...
            data1$data0$enumerators<-ruODK::entity_list(did = "Enumerator_List") %>%
              select(section_meta_interviewername=uuid,interviewername=current_version_label)
            ...
            if(nrow(data1$data0$reg$data)>0){
              data1$data0$reg$data<-data1$data0$reg$data    %>% 
                rename("section_meta_interviewerid"=section_meta_interviewername,"section_meta_interviewername"=interviewername)%>%
                select(section_meta_interviewername,
                       colnames(data1$data0$reg$data)[colnames(data1$data0$reg$data)!="section_meta_interviewername"],section_meta_interviewerid)
            ...
            data1$data0$women$data<-data1$data0$women$data %>% 
              left_join(data1$data0$enumerators)%>% 
              rename("section_meta_interviewerid"=section_meta_interviewername,"section_meta_interviewername"=interviewername)%>%
              select(section_meta_interviewername,colnames(data1$data0$women$data)[colnames(data1$data0$women$data)!="section_meta_interviewername"],section_meta_interviewerid)
            
            #merge enumerators into global data
            data1$data0$global$data<-data1$data0$global$data %>% 
              left_join(data1$data0$enumerators)%>% 
              rename("section_meta_interviewerid"=section_meta_interviewername,"section_meta_interviewername"=interviewername)%>%
              select(section_meta_interviewername,colnames(data1$data0$global$data)[colnames(data1$data0$global$data)!="section_meta_interviewername"],section_meta_interviewerid)
            
            
            #update enumerator selection menu
            updateSelectInput(session=session,"enums", 
            ...
            data1$data0$enumerators<-unique(c(unique(data1$data0$reg$data$section_meta_interviewername),
                                              unique(as.character(data1$data0$women$data$section_meta_interviewername)),
                                              unique(as.character(data1$data0$global$data$section_meta_interviewername))))
          ...
          output$data_in <- reactive({
            ...
            if(    class(data1$data0[[input$form1]]$full_data)[1]!="try-error"){
              if(nrow(data1$data0$global$full_data)>0 | nrow(data1$data0$women$full_data)>0 | nrow(data1$data0$reg$full_data)>0){
                z<-TRUE
              }  
            } 
            z
          ...
          data1$data0$geopoints<- filter(data1$data0$global$full_form,type=="geopoint")%>%
            mutate( data.frame(data="global")) %>% full_join(
              filter(data1$data0$reg$full_form,type=="geopoint")%>% mutate(data="reg"))  %>% full_join(
                filter(data1$data0$women$full_form,type=="geopoint")%>% mutate( data.frame(data="women")))
          
          if(nrow(data1$data0$geopoints)>0){
            updateSelectInput(session=session,"geo_var",choices=c(paste(data1$data0$geopoints$data,data1$data0$geopoints$ruodk_name,sep=": ")),
                              selected =paste(data1$data0$geopoints$data,data1$data0$geopoints$ruodk_name,sep=": ")[1])
          }
          else{
          ...
          colnames(data1$data0$reg$data) <- str_remove_all(colnames(data1$data0$reg$data),"_metadata")
          colnames(data1$data0$women$data) <- str_remove_all(colnames(data1$data0$women$data),"_metadata")
          colnames(data1$data0$global$data) <- str_remove_all(colnames(data1$data0$global$data),"_metadata")
          ...
          if(nrow(data1$data0$reg$data)>0){
            data1$data0$reg_id<-data1$data0$reg$data %>%
              select(submission=system_submission_date,
            ...
            data1$data0$reg_n<-data1$data0$reg_id %>%
              group_by(entity) %>%
              summarise(registrations=n(),reg_date=paste(unique(submission),collapse="; "))
            ...
            data1$data0$reg_n<-data.frame(entity="",registrations=0,reg_date=NA)
          }
          ...
          data1$data0$women_n<-data1$data0$women_id %>%
            group_by(meta_id) %>%
            summarise(womens_form=n(),womens_date=paste(unique(interview),collapse="; "))
          
          data1$data0$global_n<-data1$data0$global_id %>%
            group_by(meta_id) %>%
            summarise(global=n(),global_date=paste(unique(interview),collapse="; "))
          ...
          data1$data0$status<-data1$data0$entities %>%
            left_join(data1$data0$reg_n,
                      by=c("current_version_label"="entity"))%>%
            left_join(data1$data0$women_n,
                      by=c("uuid"="meta_id"))%>%
            left_join(data1$data0$global_n,
                      by=c("uuid"="meta_id")) %>%
            ...
            mutate(Status=case_when( womens_form==1 & global==1 ~ language_switch("Status_1",input$language),
                                     ( womens_form>1 | global>1) & complete_forms==2 ~ language_switch("Status_2",input$language),
                                     ( womens_form>1 | global>1) & (complete_forms<2) ~ language_switch("Status_4",input$language),
                                     is.na(womens_form) & is.na(global) ~ language_switch("Status_5",input$language),
                                     .default = language_switch("Status_3",input$language)
                                     
            ))%>%
            mutate(Status=factor(Status,levels=c( language_switch("Status_1",input$language),
          ...
          data1$data0$women$data <- left_join((data1$data0$women$data%>%
                                                 mutate(uuid=paste0("uuid",":",section_meta_location2_id))),
                                              select(data1$data0$status,uuid,Status))
          
          
          data1$data0$global$data <- left_join((data1$data0$global$data%>%
                                                  mutate(uuid=paste0("uuid",":",section_meta_location2_id))),
                                               select(data1$data0$status,uuid,Status))
          
          if(nrow(data1$data0$reg$data)>0){
            data1$data0$reg$data <- left_join(data1$data0$reg$data,
                                              select(data1$data0$status,meta_entity_label=current_version_label,Status))
          }
          
          #update the list of respondents based on their status
          
          updateSelectInput(session = session,"inspect",
                            choices=split(data1$data0$status$current_version_label,data1$data0$status$Status))
          data1$data0$flag<-TRUE
          messages$data_message<-language_switch("output_data_message_success",input$language)
          
          saveRDS(data1$data0,file=paste("data_objects/",project_code,"_",input$language,".RDS",sep=""))
        ...
        if(messages$progress==2){
          print(messages$progress)
          data1$data0<-readRDS(file=paste("data_objects/",project_code,"_",input$language,".RDS",sep=""))
          ...
          updateSelectInput(session=session,
                            inputId =  "question",choices=data1$data0[[input$form2]]$questionnairelist )
          ...
          #global data
          data_processed$global <- read.csv(data_directory$global[data_directory$project==project_code])%>%
            mutate_all(function(x) ifelse(x=='', NA, x))
          
          print(data_directory$global)
          
          data_processed$global <- merge_labels_clean(data_processed$global, data1$data0$global$full_form, input$language)
          
          #Women data
          data_processed$women <- read.csv(data_directory$womens[data_directory$project==project_code])%>%
            mutate_all(function(x) ifelse(x=='', NA, x))
          
          wom_assets <- data_processed$women%>%select(household_id,ends_with("own"))
          data_processed$women <- merge_labels_clean(data_processed$women, data1$data0$women$full_form, input$language)
          data_processed$women <- data_processed$women%>%select(-ends_with("own"))%>%
            left_join(wom_assets)
          incProgress(0.05)
          data_processed$indicators <- read.csv(data_directory$indicators[data_directory$project==project_code])%>%
            mutate_all(function(x) ifelse(x=='', NA, x))
          data_processed$indicators$partner<-names(proj_list)[proj_list==project_code]
          
          #Crops Data
          data_processed$crops <- read.csv(data_directory$crops[data_directory$project==project_code])%>%
            mutate_all(function(x) ifelse(x=='', NA, x))
          ...
          data_processed$crops <- merge_labels_clean(data_processed$crops, data1$data0$global$full_form, input$language)
          data_processed$crops <- merge_crops_label(data_processed$crops, data1$data0$global$full_form, input$language)
          incProgress(0.05)
          #Off farm
          data_processed$off_farm <- read.csv(data_directory$off_farm[data_directory$project==project_code])%>%
            mutate_all(function(x) ifelse(x=='', NA, x))
          
          if(!is.na(data_directory$livestock[data_directory$project==project_code])){
            data_processed$livestock <- read.csv(data_directory$livestock[data_directory$project==project_code])%>%
              mutate_all(function(x) ifelse(x=='', NA, x))
            
            data_processed$livestock <- merge_labels_clean(data_processed$livestock, data1$data0$global$full_form, input$language)
            data_processed$livestock <- merge_livestock_label(data_processed$livestock, data1$data0$global$full_form, input$language)
            
          }
          
          data_processed$hh_roster <- read.csv(data_directory$hh_roster[data_directory$project==project_code])%>%
            mutate_all(function(x) ifelse(x=='', NA, x))
        ...
        output$report <- reactive({
          file.exists(file.path(paste0("output/reports"),paste(project_code,input$language,"report.pdf",sep="_")))  & messages$loading==0       })
        
        outputOptions(output, "report", suspendWhenHidden = FALSE, priority = 10)
        
        rep_names<-c(language_switch("genReport",input$language),language_switch("prevReport",input$language))
        rep_choices<-c(0,1)
        names(rep_choices)<-rep_names
        
        if(file.exists(file.path(paste0("output/reports"),paste(project_code,input$language,"report.pdf",sep="_"))) ){
          updateSelectInput(session = session,"regen",label="", choices = rep_choices[c(2,1)])
          
        }else{
          updateSelectInput(session = session,"regen",label="", choices = rep_choices[1])
        }
    ...
    observeEvent(input$form1,{
      if(input$form1!=input$form2){
        updateSelectInput(session = session,inputId = "form2",selected = input$form1)
      }
    })
    observeEvent(input$form2,{
      if(input$form1!=input$form2){
        updateSelectInput(session = session,inputId = "form1",selected = input$form2)
      }
      updateSelectInput(session=session,
                        inputId =  "question",choices=data1$data0[[input$form2]]$questionnairelist )
    })
    ...
    output$data_indicators<-DT::renderDT({
      ...
      }else if(data_directory$Status[data_directory$project==project_code]==2){
    ...
    output$ProcessedTable <- render_gt({
      ...
      print(c(
        ...
        sum(data_processed$global$household_id%in%data_processed$women$household_id),
        sum(!data_processed$global$household_id%in%data_processed$women$household_id),
        sum(!data_processed$women$household_id%in%data_processed$global$household_id)
      ))
      
      print(report_translations[report_translations$type=="label" & report_translations$indicator=="Survey Summary", input$language])
      data.frame(
        Form = report_translations[report_translations$type=="label" & report_translations$indicator=="Survey Summary", input$language],
        N = c(
          nrow(data_processed$global),
          nrow(data_processed$women),
          sum(data_processed$global$household_id%in%data_processed$women$household_id),
        ...
        gt()%>%
        cols_label(
          Form = report_translations[report_translations$english == "Form", input$language]
        )%>%
    ...
    output$dl <- downloadHandler(
      ...
      filename = function() {
        ...
        paste0(input$forms,input$form1,data1$data0[[input$form1]]$selected_language, ".csv")
      },
    ...
    output$dlx <- downloadHandler(
      ...
      filename = function() {
        # Use the selected dataset as the suggested file name
        paste0(input$forms,input$form1,data1$data0[[input$form1]]$selected_language, ".xlsx")
      },
      content = function(file) {
        # Write the dataset to the `file` that will be downloaded
        ...
        if(length(data1$data0[[input$form1]]$repeats)>0){
          for(i in 1:length(data1$data0[[input$form1]]$repeats)){
            out_xl[[substr(names(data1$data0[[input$form1]]$repeats)[i],1,30)  ]]<-data1$data0[[input$form1]]$repeats[[i]]  
          }
        }
    ...
    output$dlz <- downloadHandler(
      ...
      content = function(file) {
        ...
        for(i in 1:length(data1$data0[["global"]]$repeats)){
          out_xl[[substr(names(data1$data0[["global"]]$repeats)[i],1,30)  ]]<-data1$data0[["global"]]$repeats[[i]]  
        }
        
        write.xlsx(out_xl,paste0(tmp,"/mydir/",input$forms,"global",data1$data0[["global"]]$selected_language, ".xlsx"))
        ...
        for(i in 1:length(data1$data0[["women"]]$repeats)){
          out_xl2[[substr(names(data1$data0[["women"]]$repeats)[i],1,30)  ]]<-data1$data0[["women"]]$repeats[[i]]  
        }
        
        write.xlsx(out_xl2,paste0(tmp,"/mydir/",input$forms,"women",data1$data0[["women"]]$selected_language, ".xlsx"))
        
        out_xl3<-list()
        out_xl3[["main_data"]]<-data1$data0[["reg"]]$data
        
        for(i in 1:length(data1$data0[["reg"]]$repeats)){
          out_xl3[[substr(names(data1$data0[["reg"]]$repeats)[i],1,30)  ]]<-data1$data0[["reg"]]$repeats[[i]]  
        }
        
        write.xlsx(out_xl3,paste0(tmp,"/mydir/",input$forms,"reg",data1$data0[["reg"]]$selected_language, ".xlsx"))
      ...
      output$MonitorMap<-renderLeaflet({  
        if(input$geo_var!="0"){  
          ...
          geo_data<-data1$data0[[geo_var[1]]]$data %>%
            select(all_of(c("id","Status","system_submission_date",geo_var[2]))) %>%
            na.omit() %>%
            st_as_sf(wkt = geo_var[2])
          
          pal<-colorFactor(
            palette = "Dark2",
            domain = c(language_switch("Status_1",input$language),
                       language_switch("Status_2",input$language),
                       language_switch("Status_3",input$language),
                       language_switch("Status_4",input$language),
                       language_switch("Status_5",input$language))
          ...
          leaflet(geo_data) %>%
            addProviderTiles("OpenStreetMap.Mapnik", group = "OSM (default)",options = providerTileOptions(opacity=0.5)) %>%
            addProviderTiles("OpenTopoMap", group = "Topography",options = providerTileOptions(opacity=0.5)) %>%
            addProviderTiles("Esri.WorldImagery", group = "World Imagery (satellite)",options = providerTileOptions(opacity=0.5)) %>%
            # Layers control
            addLayersControl(
              baseGroups = c(
                "OSM (default)",
            ...
            addCircles(popup= ~paste(Status,id,as.character(system_submission_date),sep="\n"),
                       color = ~pal(Status),opacity=0.75
                       #,clusterOptions = circleClusterOptions()
            )%>%
            addLegend(pal = pal, values = c(language_switch("Status_1",input$language),
                                            language_switch("Status_2",input$language),
                                            language_switch("Status_3",input$language),
                                            language_switch("Status_4",input$language),
                                            language_switch("Status_5",input$language))
    ...
    output$inspect_summary<-render_gt({
      f1<-filter(data1$data0$status,current_version_label==input$inspect) %>% 
        ungroup() %>%
        select(current_version_label,Status,complete_forms,
               "Global Forms"=global,"Global Date(s): "=global_date,
               "Women's Forms"=womens_form,"Women' Date(s): "=womens_date)
      colnames(f1)[4:7]<-c(  language_switch("select_form1_global",input$language),
    ...
    output$inspect_i<- DT::renderDataTable({
      ...
      resps<- filter(data1$data0[[input$form_i]]$data,uuid%in%filter(data1$data0$status,current_version_label==input$inspect)$uuid)
      ...
      if(nrow(resps)>0){
        ...
        resps%>% t() %>%
          ...
          inner_join(select(data1$data0[[input$form_i]]$form,ruodk_name,any_of(paste0("label_",input$language)))) %>%
          select(any_of(paste0("label_",input$language)),any_of(paste0("V",1:9))) %>%
          return()
      }
        ...
        if(input$monitor_table=="completion"){
          ...
          if(nrow(data1$data0$reg$data)>0){  
            reg_sum<-data1$data0$reg$data %>%
              select(submission=system_submission_date,
                     ...
                     entity=meta_entity_label,
                     instance=meta_instance_id)%>%
              summarise(Form=language_switch("select_form1_reg",input$language),
                        "Submissions"=n(),
            ...
            #form level summary
            reg_sum<-
              data.frame(Form=language_switch("select_form1_reg",input$language),
                         "Submissions"=0,
                         "Start Date"=NA,
                         "End Date"=NA,
                         "Average Time"=NA,
          ...
          women_sum<-data1$data0$women$data %>%
            select(submission=system_submission_date,
                   ...
                   section_meta_location2_id)%>%
            summarise(Form=language_switch("select_form1_women",input$language),
                      "Submissions"=n(),
                      "Start Date"=min(date(interview),na.rm=T),
                      "End Date"=max(date(interview),na.rm=T),
                      "Average Time"=round(median( (as.numeric(end)-as.numeric(start)) / 60,na.rm=T),1),
                      "Interviews < 10 minutes"=scales::percent(mean(((as.numeric(end)-as.numeric(start)))<(60*10),na.rm=T))
            )
          ...
          global_sum<-data1$data0$global$data %>%
            select(submission=system_submission_date,
                   ...
                   section_meta_location2_id) %>%
            summarise(Form=language_switch("select_form1_global",input$language),
                      "Submissions"=n(),
                      "Start Date"=min(date(interview),na.rm=T),
                      "End Date"=max(date(interview),na.rm=T),
                      "Average Time"=round(median( (as.numeric(end)-as.numeric(start)) / 60,na.rm=T),1),
          ...
          colnames(reg_sum)<-colnames(global_sum)<-colnames(women_sum)<-c("Form",language_switch("label_submissions",input$language),
    ...
    output$enum_summary<-renderDataTable({
      if(data1$data0$flag==TRUE){
        ...
        women_sum<-data1$data0$women$data %>%
          ...
          filter(interview>=as.Date(input$dates)[1] & interview<=as.Date(input$dates)[2]) %>%
        ...
        # colnames(reg_sum)<-
        colnames(global_sum)<-colnames(women_sum)<-c("Enumerator","Form",language_switch("label_submissions",input$language),
                                                     language_switch("label_start",input$language),
                                                     language_switch("label_end",input$language),
                                                     language_switch("label_time",input$language),
                                                     language_switch("label_interview10",input$language))
    ...
    output$SummaryTable<-render_gt({
      ...
      if(data1$data0$flag==TRUE){
        variable<-filter(data1$data0[[input$form2]]$form,label==input$question)$ruodk_name
        
        #use defined odk_table function to detect class of variable and produce a relevant table
        odk_table(data1$data0[[input$form2]]$data,variable,data1$data0[[input$form2]]$form,
    ...
    output$indicator_text <- renderText({
      ...
      if(str_detect(input$indicator, "FIES")){
        ...
        HTML(paste0(
          report_translations[report_translations$element=="extra_text3", input$language], "</br>","</br>",
          report_translations[report_translations$element=="extra_text4", input$language], "</br>","</br>",
          "<ul>",
          "<li>", report_translations[report_translations$element=="extra_text5", input$language],"</li>",
          "<li>", report_translations[report_translations$element=="extra_text6", input$language],"</li>",
          "<li>", report_translations[report_translations$element=="extra_text7", input$language],"</li>",
          "<li>", report_translations[report_translations$element=="extra_text8", input$language],"</li>",
          "</ul>"
        ))
        ...
        HTML(paste0(
          report_translations[report_translations$element=="extra_text1", input$language], "</br>","</br>",
          report_translations[report_translations$element=="extra_text2", input$language]
        ))
        
      }else if(str_detect(input$indicator, "simpson")){
        
        HTML(paste0(
          report_translations[report_translations$element=="extra_text11", input$language], "</br>","</br>",
          report_translations[report_translations$element=="extra_text12", input$language]
        ))
    ...
    output$IndicatorTable <- render_gt({
      ...
      if(data_directory$Status[data_directory$project==project_code]==1){
        indicator_table(data1$data0$indicators, input$indicator, indicator_directory, input$groupvar, input$language)%>%
          as_gt()
      }else{
        indicator_table(data_processed$indicators, input$indicator, indicator_directory, input$groupvar, input$language)%>%
          as_gt()
      }
    ...
    output$IndicatorPlot <- renderPlotly({
      ...
      if(data_directory$Status[data_directory$project==project_code]==1){
        indicator_plot(data1$data0$indicators, input$indicator, indicator_directory,
                       input$groupvar, language = input$language)
      }else{
        indicator_plot(data_processed$indicators, input$indicator, indicator_directory,
                       input$groupvar, language = input$language)
      }
    ...
    observeEvent(input$question,ignoreInit = TRUE,{
      ...
      output$select1 <- reactive({
        filter(data1$data0[[input$form2]]$form,label==input$question)$type=="select" & 
          is.na(filter(data1$data0[[input$form2]]$form,ruodk_name==
                         filter(data1$data0[[input$form2]]$form,label==input$question)$ruodk_name)$selectMultiple)
      })
    ...
    output$SummaryPlot<-renderPlot({
      if(data1$data0$flag==TRUE){
        ...
        if( filter(data1$data0[[input$form2]]$form,label==input$question)$type=="select"){
          
          if(is.na(filter(data1$data0[[input$form2]]$form,label==input$question)$selectMultiple)){
        ...
        p2<- odk_plot(data1$data0[[input$form2]]$data,variable,data1$data0[[input$form2]]$form,repeats=data1$data0[[input$form2]]$repeats,
                      plotvar = plotvar) 
        print(p2)
      }
    })
    ...
    observeEvent(input$genBaiscReport,priority = 1, {
      withProgress(message = language_switch("output_report_generating",input$language), {
        
        if(messages$progress<2){
          
          print(1)
          if(!dir.exists(outpath$outpath)){dir.create( outpath$outpath,recursive = TRUE)}
          
          tempReport <- file.path(outpath$outpath,paste(project_code,input$language,"report.Rmd",sep="_"))
          htmlReport <- file.path(outpath$outpath,paste(project_code,input$language,"report.html",sep="_"))
          pdfReport <- file.path(outpath$outpath,paste(project_code,input$language,"report.pdf",sep="_"))
        ...
        if(messages$progress==2 & 
           ( !file.exists(file.path( outpath$outpath,paste(project_code,input$language,"report.pdf",sep="_"))) |
             input$regen==0)){
        ...
        if((messages$progress==2 & 
            ( !file.exists(file.path( outpath$outpath,paste(project_code,input$language,"report.pdf",sep="_"))) |
              input$regen==0))|messages$progress<2){
          print(3)
          html_fn <- rmarkdown::render(tempReport,
                                       rmarkdown::html_document(theme = bslib::bs_theme("cosmo",version=4)),
                                       params = params,
                                       output_file = htmlReport,
                                       envir = new.env(parent = globalenv()))
          
          pagedown::chrome_print(html_fn,pdfReport, options=list(printBackground=TRUE))
      ...
      output$printBasicReport2 <- downloadHandler(
        ...
        content = function(file){
          file.copy(file.path( outpath$outpath,paste(project_code,input$language,"report.pdf",sep="_")),file)
        })
      messages$loading<-0
    })
