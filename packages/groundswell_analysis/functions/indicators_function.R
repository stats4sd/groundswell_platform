...
derive_indicators <- function(
  ...
  women_form$choices <- ifelse(
    women_form$choices %in% c("NA", "NULL"),
    NA,
    women_form$choices
  )

  women_form$type <- ifelse(
    !is.na(women_form$choices) &
      !is.null(women_form$choices) &
      women_form$type == "string",
    "select",
    women_form$type
  )
  ...
  selects <- filter(
    women_form,
    type == "select" & is.na(selectMultiple) & ruodk_name %in% colnames(women)
  )

  xx <- colnames(
    women %>%
      select(
        survey_grp_womens_section_vcc_module_voice_group_voice_hh_food,
  ...
  global_form$choices <- ifelse(
    global_form$choices %in% c("NA", "NULL"),
    NA,
    global_form$choices
  )

  global_form$type <- ifelse(
    !is.na(global_form$choices) &
      !is.null(global_form$choices) &
      global_form$type == "string",
    "select",
    global_form$type
  )
  ...
  selects <- filter(
    global_form,
    type == "select" & is.na(selectMultiple) & ruodk_name %in% colnames(global)
  )
  ...
  cols_global_ruodk <- data.frame(
    ruodk_name = global_form$ruodk_name,
  ...
  cols_women_ruodk <- data.frame(
    ruodk_name = women_form$ruodk_name,
  ...
  crop_form <- global_form %>%
    filter(str_detect(ruodk_name, "crop_repeat")) %>%
    filter(name != "crop_repeat") %>%
  ...
  extra_vars_women <- extra_vars[
    extra_vars %in% colnames(women) & !extra_vars %in% extra_vars_global
  ]
  ...
  indicators <-
    full_join(
      global %>%
        select(household_id, all_of(extra_vars_global), "global_form_id" = id),
      women %>%
        select(household_id, all_of(extra_vars_women), "womens_form_id" = id)
    ) %>%
  ...
  factors <- data.frame(
    filter(global_form, name == "crop_list")$`choices_english_(en)`
  ) %>%
    distinct()
  crop_lists <- data.frame(
    global_form_id = global$id,
    str_split_fixed(global[["crop_list"]], " ", nrow(factors))
  )

  crop_count <- crop_lists %>%
    mutate_at(vars(-global_form_id), function(x) {
      ifelse(is.na(x) | x == "", 0, 1)
    }) %>%
    mutate(
      I_P_total_crops = rowSums(select(., starts_with("X")), na.rm = TRUE)
    ) %>%
  ...
  crop_count <- crop_count %>%
    left_join(crops_important) %>%
    mutate(
      I_P_total_important_crops = ifelse(
        grow_crops == "N",
        0,
        I_P_total_important_crops
      )
    )
  ...
  indicators <- indicators %>%
    left_join(
      crop_count %>%
        select(global_form_id, I_P_total_crops, I_P_total_important_crops)
    )
  ...
  staple <- crops %>%
    ...
    mutate(
      staple = ifelse(
        label_lang %in% c("Millet", "Maize", "Sorghum", "Wheat"),
        "Staple",
        "Other"
      )
    ) %>%
    ...
    summarise(
      staple_area = sum(as.numeric(crop_area_ha), na.rm = TRUE),
  ...
  crop_area_prod <- crops %>%
    ...
    group_by("global_form_id" = submissions_id) %>%
    summarise(
      I_P_total_crop_area = sum(crop_area_ha, na.rm = TRUE),
      I_P_total_crop_yield = sum(crop_yield_kg, na.rm = TRUE)
    ) %>%
    mutate(
      I_P_total_crop_productivity = I_P_total_crop_yield / I_P_total_crop_area
    )
  ...
  simpson <- crops %>%
    ...
    summarise(num = sum(num), denom = mean(denom)) %>%
    mutate(D = num / denom) %>%
  ...
  if ("livestock_owners" %in% colnames(global)) {
    factors <- data.frame(
      filter(global_form, name == "livestock_all")$`choices_english_(en)`
    ) %>%
      distinct()
    livestock_lists <- data.frame(
      global_form_id = global$id,
      str_split_fixed(global[["livestock_all"]], " ", nrow(factors))
    )
    ...
    livestock_count <- livestock_lists %>%
      ...
      mutate(
        I_P_total_livestock = rowSums(select(., starts_with("X")), na.rm = TRUE)
      ) %>%
      left_join(global %>% select("global_form_id" = id, livestock_owners)) %>%
      mutate(
        I_P_total_livestock = ifelse(
          livestock_owners == "N",
          0,
          I_P_total_livestock
        )
    ...
    livestock_count <- livestock_count %>%
      left_join(livestocks_important) %>%
      mutate(
        I_P_total_important_livestocks = ifelse(
          livestock_owners == "N",
          0,
          I_P_total_important_livestocks
        )
      )
  ...
  vcc <- women %>%
    select(
      "womens_form_id" = id,
      "I_VCC1" = voice_hh_food,
      "I_VCC1_bin" = voice_hh_food,
      "I_VCC2" = voice_hh_crops,
      "I_VCC2_bin" = voice_hh_crops,
      "I_VCC3" = voice_hh_spending,
      "I_VCC3_bin" = voice_hh_spending,
      "I_VCC4" = choice_hh_income_women,
      "I_VCC4_bin" = choice_hh_income_women,
      "I_VCC5" = choice_comm_market,
      "I_VCC5_bin" = choice_comm_market,
      "I_VCC6" = choice_comm_committee,
      "I_VCC6_bin" = choice_comm_committee
    )

  indicators <- indicators %>%
    left_join(
      vcc %>%
        select(womens_form_id, I_VCC1, I_VCC2, I_VCC3, I_VCC4, I_VCC5, I_VCC6)
    )
  ...
  assets <- women %>%
    ...
    mutate(
      ...
      total_owned_assets = rowSums(
        select(., ends_with("own")) < 3,
        na.rm = TRUE
      ),
      total_minor_assets = as.numeric(!is.na(poultry_own)) +
        as.numeric(!is.na(non_mech_eqp_own)) +
        as.numeric(!is.na(small_consumer_own)),
      total_major_assets = total_owned_assets - total_minor_assets
    ) %>%
    mutate(
      I_AWEAI_asset = ifelse(
        total_major_assets >= 1 | total_minor_assets >= 2,
        1,
        0
      ),
      I_AWEAI_asset_prop = (total_major_assets + total_minor_assets) /
        total_assets
    )

  indicators <- indicators %>%
    left_join(
      assets %>% select(womens_form_id, I_AWEAI_asset, I_AWEAI_asset_prop)
    )
  ...
  global$crop_only <- ifelse(
    global$grow_crops == "Y" & !global$id %in% off_farm$submissions_id,
    1,
    0
  )
  global$crop_plus_offarm <- ifelse(
    global$grow_crops == "Y" & global$id %in% off_farm$submissions_id,
    1,
    0
  )
  global$income_unreported <- ifelse(
    global$grow_crops == "N" & !global$id %in% off_farm$submissions_id,
    1,
    0
  )
  global$off_farm_only <- ifelse(
    global$grow_crops == "N" & global$id %in% off_farm$submissions_id,
    1,
    0
  )
  ...
  finance <- global %>%
    ...
    mutate(
      I_HI_self_perception = case_when(
        finance_1 == "Y" ~ 7,
        finance_2 == "Y" ~ 6,
        finance_3 == "Y" ~ 5,
        finance_4 == "Y" ~ 4,
  ...
  indicators <- indicators %>%
    left_join(
      finance %>% select(household_id, global_form_id, starts_with("I_"))
    )
    ...
    left_join(
      women %>%
        select(
          household_id,
          "womens_form_id" = id,
          "I_HI_transport" = transportation_hh
        )
    )
  ...
  hdds <- women %>%
    ...
    mutate(
      I_HDDS = rowSums(
        select(., Grains_24hr:condiments_24hr) == "Y",
        na.rm = TRUE
      )
    ) %>%
  ...
  fies <- women %>%
    ...
    mutate_at(vars(starts_with("FIES")), function(x) {
      case_when(
        x == "Y" ~ 1,
        x == "N" ~ 0,
        .default = NA
  ...
  fies_results <- RM.w(
    .data = as.matrix(fies %>% select(starts_with("FIES"))),
  ...
  item_parameters$after <- (item_parameters$before - mean_standard) /
    std.dev_standard *
    std.dev_country +
    mean_country
  ...
  scores$percentage <- raw_score_parameters$w_cases /
    sum(raw_score_parameters$w_cases)
  mod_severe <- item_parameters$after[5]
  severe <- item_parameters$after[8]

  scores$probability_mod_sev <- ifelse(
    scores$score == 0,
    0,
    1 -
      pnorm(
        mod_severe,
        raw_score_parameters$severity,
        raw_score_parameters$error
      )
  )
  scores$probability_sev <- ifelse(
    scores$score == 0,
    0,
    1 - pnorm(severe, raw_score_parameters$severity, raw_score_parameters$error)
  )
  ...
  fies <- fies %>%
    mutate(
      I_FIES_score = rowSums(select(., starts_with("FIES")), na.rm = TRUE)
    ) %>%
    left_join(
      scores %>%
        select(
          score,
          "I_FIES_prob_moderate" = probability_mod_sev,
          "I_FIES_prob_severe" = probability_sev
        ),
  ...
  indicators$I_FIES_category <- case_when(
    ...
    (coalesce(indicators$FIES_1, 0) +
      coalesce(indicators$FIES_2, 0) +
      coalesce(indicators$FIES_3, 0) +
      coalesce(indicators$FIES_4, 0)) >
      0 &
      (coalesce(indicators$FIES_5, 0) +
        coalesce(indicators$FIES_6, 0) +
        coalesce(indicators$FIES_7, 0) +
        coalesce(indicators$FIES_8, 0)) ==
        0 ~ "Mildly Food Insecure",
    (coalesce(indicators$FIES_5, 0) +
      coalesce(indicators$FIES_6, 0) +
      coalesce(indicators$FIES_7, 0)) >
      0 &
      coalesce(indicators$FIES_8, 0) == 0 ~ "Moderately Food Insecure",
