  ...
  theme_bw() %+replace%    #replace elements we want to change
    ...
    theme(
      ...
      plot.title = element_text(             #title
        ...
        hjust = 0,                #left align
        vjust = 2,
        colour = "#000"),               #raise slightly
      
      plot.subtitle = element_text(          #subtitle
        #family = font,            #font family
        size = 14,
        colour = "#000"),               #font size
      
      plot.caption = element_text(           #caption
        #family = font,            #font family
        size = 9,                 #font size
        hjust = 1,
        colour = "#000"),               #right align
...
indicator_table <- function(data, indicator, indicator_directory, groupvar, language,translation_file = report_translations){
  
  type <- indicator_directory$type[indicator_directory$indicator_name == indicator]
  
  ignore_respondent_gender <- indicator_directory$ignore_respondent_gender[indicator_directory$indicator_name == indicator]
  ...
  indicator_name <- indicator_directory[indicator_directory$indicator_name==indicator, language]
  ...
  if(type %in% c("decimal", "integer", "percent")){
    ...
    if(type=="binary"){
      ...
      table <- data%>%
        mutate(indicator = factor(indicator,levels = c(0,1),
                                  labels = translation_file[translation_file$indicator=="binaries", language]))%>%
        tbl_summary(
          by = groupvar,
          include = indicator,
          type = indicator~"categorical",
...
indicator_plot <- function(data, indicator, indicator_directory, groupvar, language, translation_file = report_translations) {
  ...
  indicator_name <- str_wrap(indicator_directory[indicator_directory$indicator_name==indicator, language],45)
  ...
  if (type %in% c("decimal", "integer")) {
    ...
    if(bins > 13 & type == "integer") {
     ...
     ht <- paste0(
         report_translations[report_translations$`english_(en)` == "Indicator", language],
         ": %{x}<br>",
         report_translations[report_translations$`english_(en)` == "Number of Households", language],
         ": %{y}<extra></extra>"
       )
     ...
     plot$x$data <- lapply(plot$x$data, function(trace) {
      ...
      plot <- data %>%
        ggplot(aes(x = indicator)) +
        geom_histogram(fill = groundswell_palette[2], colour = "black", boundary = 0)+
        scale_y_continuous(n.breaks = 10)+
        facet_wrap(~groupvar)+
        theme_groundswell()+
        labs(
      ...
      ht <- paste0(report_translations[report_translations$`english_(en)`=="Indicator", language],
                   ": %{x}<br>",
                   report_translations[report_translations$`english_(en)`=="Number of Households", language],
                   ": %{y}<extra></extra>")
      ...
      plot <- data %>%
        ...
        summarise(n = n(),
                  n_lab = paste0("n = ", n))%>%
        ggplot(aes(x = indicator, y = n,
                   text = paste0(
                     report_translations[report_translations$`english_(en)`=="Indicator", language][1], ": ", indicator,
                     "<br>",
                     report_translations[report_translations$`english_(en)`=="Number of Households", language][1], ": ", n
                   ))) +
  ...
  if(type == "categorical"){
    ...
    plot <- data %>%
      ...
      summarise(n = n(),
                ...
                perc = n/N,
                n_lab = paste0("n = ", n))%>%
      ggplot(aes(y = indicator, x = perc,
                 text = paste0(
        report_translations[report_translations$`english_(en)`=="Indicator", language][1], ": ", indicator,
        "<br>",
        report_translations[report_translations$`english_(en)`=="Percentage of households", language][1],": ",
      ...
      geom_col(fill = groundswell_palette[2], colour = "black")+
      geom_label(aes(label = n_lab,
                     x = ifelse(perc < 0.05, 0.1,
                                ifelse(perc > 0.9, 0.85, perc))))+
      facet_wrap(~groupvar)+
      scale_x_continuous(labels = scales::percent, limits = c(0,1))+
      #scale_y_discrete(labels = str_wrap(levels(indicator),25))+
      theme_groundswell()+
      labs(y = indicator_name,
  ...
  if(type == "binary"){
    ...
    plot <- 
      ...
      summarise(n = n(),
                perc = n/max(N),
                n_lab = paste("n = ", n))%>%
      ggplot(aes(x = groupvar, y = perc, fill = as.factor(indicator),
                 text = paste0(
                   report_translations[report_translations$`english_(en)`=="Indicator", language][1], ": ", indicator,
                   "<br>",
                   report_translations[report_translations$`english_(en)`=="Percentage of households", language][1],": ",
      ...
      geom_label(aes(label = n_lab), position = position_dodge(width = 0.9),show.legend = FALSE)+
      theme_groundswell()+
      ...
      scale_fill_manual(values = c(groundswell_palette[7], groundswell_palette[4]))
