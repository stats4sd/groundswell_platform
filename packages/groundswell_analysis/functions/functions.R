generate_svc <- function(
  server = "https://odk.stats4sd.org",
  projID,
  xmlFormID
) {
  svc = paste0(
...
language_file1 <- read.xlsx(
  "inputs/groundswell_monitor_text_fortranslation_translated.xlsx"
)
...
merge_labels <- function(variable, data, form, repeatgrp = FALSE) {
  ...
  if (nrow(factors) > 0) {
    out_of_list <- na.omit(data[[variable]][
      !data[[variable]] %in% factors$values
    ])
...
merge_labels_clean_indicator <- function(
  variable,
  indicator,
  data,
  form,
  ...
  form$type <- ifelse(
    !is.na(form$choices) & !is.null(form$choices) & form$type == "string",
    "select",
    form$type
  )
  ...
  if (nrow(factors) > 0) {
    out_of_list <- na.omit(data[[indicator]][
      !data[[indicator]] %in% factors$values
    ])
...
merge_labels_clean <- function(data, form, language) {
  ...
  selects <- filter(
    form,
    type == "select" & is.na(selectMultiple) & name %in% colnames(data)
  )
...
merge_crops_label <- function(data, form, language) {
  ...
  form$choices[form$name == "crop_label"] <- form$choices[
    form$name == "crops_important"
  ]

  factors <- data.frame(filter(form, name == "crop_label")$choices)

  out_of_list <- na.omit(data[["crop_id"]][
    !data[["crop_id"]] %in% factors$values
  ])

  data[["label_lang"]] = factor(
    data[["crop_id"]],
    levels = c(out_of_list, factors$values),
    labels = c(out_of_list, factors$labels)
  )
...
merge_livestock_label <- function(data, form, language) {
  ...
  form$choices[form$name == "livestock_label"] <- form$choices[
    form$name == "livestock"
  ]

  factors <- data.frame(filter(form, name == "livestock_label")$choices)

  out_of_list <- na.omit(data[["livestock_id"]][
    !data[["livestock_id"]] %in% factors$values
  ])
...
odk_number_table <- function(data, variable, group = NA) {
  ...
  data %>%
    summarise(
      "Responses" = length(variable[!is.na(variable)]),
...
odk_select1_table <- function(data, variable, form, group = NA, style = TRUE) {
  ...
  if (style == TRUE) {
    ...
    table1 <- table1 %>%
      mutate(
        "Percent of Non-Missing" = scales::percent(`Percent of Non-Missing`)
      ) %>%
  ...
  if (nrow(empty_set) > 0) {
    table1 <- table1 %>%
      ...
      mutate(
        response = factor(as.character(response), levels = factors$labels)
      ) %>%
      arrange(response)
  }
  table1
...
odk_selectn_table <- function(data, variable, form, group = NA, style = TRUE) {
  ...
  variables <- data.frame(
    ID = data$id,
    str_split_fixed(data[[variable]], " ", nrow(factors))
  )
  #print(variables)
  #print(factors)
  ...
  variables <- variables %>%
    ...
    ungroup() %>%
    mutate(
      "% of Responses" = (Responses / sum(Responses)),
  ...
  if (style == TRUE) {
    variables <- variables %>%
      mutate(
        "% of Responses" = scales::percent(`% of Responses`),
        "% of Question Respondents" = scales::percent(
...
odk_plot <- function(
  ...
  if (
    variable_type %in%
      c("string", "decimal", "int", "dateTime", "date", "select")
  ) {
    if (variable_type == "string") {
      p1 <- NULL
    }
...
read_clean_data <- function(
  ...
  data1$full_data <- try(ruODK::odata_submission_get(
    fid = fid,
    download = FALSE,
    wkt = TRUE,
    expand = FALSE #,
  ...
  data1$full_data <- data1$full_data %>%
    filter(
      data1$full_data$system_review_state != "rejected" |
        is.na(data1$full_data$system_review_state)
    )

  if (nrow(repeats) > 0) {
    data1$repeats <- list()
    data1$repeats$list_name <- str_remove(repeats$name, "Submissions.") %>%
      str_replace_all(fixed("."), "_") %>%
      tolower()

    for (i in 1:nrow(repeats)) {
      data1$repeats[[data1$repeats$list_name[
        i
      ]]] <- try(ruODK::odata_submission_get(
        fid = fid,
      ...
      data1$repeats[[data1$repeats$list_name[
        i
      ]]] <- data1$repeats[[data1$repeats$list_name[i]]] %>%
  ...
  if (class(data1$full_data)[1] != "try-error") {
    ...
    if (nrow(data1$full_data) > 0) {
      if (
        !"FIES_1" %in% data1$full_form$name &
          !str_detect(data1$full_data$odata_context[1], "groundswell_farm_reg")
      ) {
        if (
          "survey_grp_section_household_info_household_head_details_name_sex_calculated" %in%
            colnames(data1$full_data)
        ) {
          ...
          data1$full_data$household_head_sex <- ifelse(
            data1$full_data$survey_grp_section_household_info_respondent_details_participant_is_head ==
              "Y",
            ...
            data1$repeats$survey_grp_section_household_info_householdpopulation_hh_pop_repeat$hh_pop_repeat_grp_person_gender[
              data1$repeats$survey_grp_section_household_info_householdpopulation_hh_pop_repeat$submissions_id ==
                data1$full_data$id &
                data1$repeats$survey_grp_section_household_info_householdpopulation_hh_pop_repeat$hh_pop_rep_num ==
                  1
            ]
          )
          ...
          if (
            any(
              str_detect(colnames(data1$full_data), "participant_age_update") &
                data1$full_data$section_meta_location2_country[1] == "Nepal"
            )
          ) {
        ...
        if ("participant_age_update" %in% data1$full_form$name) {
          ...
          data1$full_data$Age_Group2 <- cut(
            as.numeric(
              data1$full_data$survey_grp_section_household_info_respondent_details_participant_age_update
            ),
          ...
          data1$full_data$Age_Group <- cut(
            as.numeric(
              data1$full_data$survey_grp_section_household_info_respondent_details_participant_age
            ),
            breaks = c(18, 26, 55, 99),
            labels = c("18 - 25", "26 - 55", "55+"),
            ordered_result = TRUE
      ...
      if ("FIES_1" %in% data1$full_form$name) {
        data1$full_data$Womens_Age_Group <- cut(
          as.numeric(
            data1$full_data$survey_grp_womens_section_respondent_details_respondentage
          ),
          breaks = c(18, 26, 55, 99),
          labels = c("18 - 25", "26 - 55", "55+"),
          ordered_result = TRUE
        ...
        data1$full_data$Womens_Age_Group2 <- cut(
          as.numeric(
            data1$full_data$survey_grp_womens_section_respondent_details_respondentage
          ),
          breaks = c(18, 31, 50, 99),
    ...
    data1$form <- filter(
      data1$form,
      !is.na(data1$form$label) | data1$form$type == "structure"
    )

    data1$form$type <- ifelse(
      !is.na(data1$form$choices) &
        data1$form$type == "string",
      "select",
      data1$form$type
    )
    if (nrow(repeats) > 0) {
      ...
      data1$form$repeats <- str_detect(
        tolower(data1$form$path),
        paste(
          paste0(
            "/",
            str_remove(tolower(repeats$name), "submissions.") %>%
      ...
      data1$form$repeat_name <- ifelse(
        data1$form$repeats == FALSE,
        NA,
        str_remove_all(
          data1$form$ruodk_name,
          paste(paste0(rep_names, "_"), collapse = "|")
        )
      )

      data1$form$repeat_data <- ifelse(
        data1$form$repeats == FALSE,
        NA,
    ...
    for (i in 1:nrow(selects)) {
      if (selects$repeats[i] == FALSE) {
        if (length(na.omit(data1$data[[selects$ruodk_name[i]]])) > 0) {
          data1$data <- merge_labels(
            selects$ruodk_name[i],
            data = data1$data,
            form = data1$form
          )
        }
      ...
      if (selects$repeats[i] == TRUE) {
        if (
          length(na.omit(data1$repeats[selects$repeat_data[i]][[
            1
          ]][[selects$repeat_name[i]]])) >
            0
        ) {
          data1$repeats[selects$repeat_data[i]][[1]] <- merge_labels(
            selects$repeat_name[i],
    ...
    data1$any <- colnames(data1$data)[
      (colSums(is.na(data1$data)) != nrow(data1$data))
    ]
      ...
      if (!is.null(data1$repeats[[i]])) {
        if (nrow(data1$repeats[[i]]) > 0) {
          data1$any <- c(
            ...
            paste(
              i,
              colnames(data1$repeats[[i]])[
                (colSums(is.na(data1$repeats[[i]])) != nrow(data1$repeats[[i]]))
              ],
              sep = "_"
            )
          )
    ...
    data1$questionnairelist <- split(
      data1$form$label,
      factor(data1$form$group_label, levels = unique(data1$form$group_label))
    ) %>%
